...
bool pointing_device_driver_init(void) {
    ...
    /* Unconditionally true, as before the TPS43 work: azoteq_iqs5xx_init()
     * reports failure if any of its several I2C writes misses the IQS5xx comm
     * window, which happens often enough that propagating it would flag the
     * device INIT_FAILED on an otherwise healthy pad. */
    return true;
}
...
static report_mouse_t sval_azoteq_read_report(void) {
...
report_mouse_t pointing_device_driver_get_report(report_mouse_t mouse_report) {
    if (timer_elapsed(azo_timer) >= AZO_MS) {
        ...
        mouse_report = sval_azoteq_read_report();
